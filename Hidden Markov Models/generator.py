...
class HMM_TxtGenerator:
    ...
    def generate_sentence(self, sentence_length):
        ...
        sent = []
        z = np.random.choice(K, p=pi.flatten())
        for i in range(sentence_length):
            x = np.random.choice(V, p=B[z, :].flatten())
            sent.append(self.word_list[x])
            z = np.random.choice(K, p=A[z, :].flatten())
