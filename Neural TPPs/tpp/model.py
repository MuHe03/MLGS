...
class NeuralTPP(torch.nn.Module):
    ...
    def __init__(self, hidden_dim: int = 16):
        ...
        self.embedding_rnn = torch.nn.RNN(
            input_size=2,
    ...
    def get_log_density(
        ...
        #######################################################
        # write here and replace the default
        prob = distribution.log_prob(times)
        len = mask.sum(dim=1)
        for i in range(mask.shape[0]):
            prob[i, len[i] - 1:] = 0
        log_density = prob.sum(dim=1)
        #######################################################
        return log_density
    ...
    def get_log_survival_prob(
        ...
        #######################################################
        # write here and replace the default
        prob = torch.log(1 - distribution.cdf(times))
        len = mask.sum(dim=1)
        log_surv_last = torch.zeros(mask.shape[0])
        for i in range(mask.shape[0]):
            log_surv_last[i] = prob[i, len[i] - 1]
        #######################################################
    ...
    def encode(
        ...
        # x = (t, log(t))
        x = torch.stack([times, torch.log(times)], dim=-1)
    ...
    def embed_history(
        ...
        # write here and replace the default
        x = self.encode(times)
        history_emb, c_n = self.embedding_rnn(x)
    ...
    def get_distribution_parameters(
        ...
        #######################################################
        # write here and replace the default
        mu, log_sigma = self.linear(history_emb).unbind(dim=-1)
