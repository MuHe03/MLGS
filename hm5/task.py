...
class GCN(nn.Module):
    ...
    def forward(self, X: torch.Tensor, A: torch.sparse.FloatTensor) -> torch.tensor:
        ...
        for (name, layer) in enumerate(self.propagate):
            X = F.relu(layer((X, A_hat)))
...
class PowerIterationPageRank(nn.Module):
    ...
    def forward(self, logits: torch.Tensor, A_hat: torch.sparse.FloatTensor) -> torch.tensor:
        ...
        for i in range(n_propagation):            
            logits = (1 - alpha) * sp.mm(sparse_dropout(A_hat, dropout, self.training), logits) + alpha * H_0
...
class APPNP(GCN):
    ...
    def forward(self, X: torch.Tensor, A: torch.sparse.FloatTensor) -> torch.tensor:
        ...
        H = self.transform_features(X)
        
        H = self.propagate(H, A_hat)
...
def calc_accuracy(logits: torch.Tensor, labels: torch.Tensor, idx_test: np.ndarray) -> float:
    ...
    _, predicted = torch.max(logits[idx_test], 1)
    correct = (predicted == labels[idx_test]).sum().item()
